"""
Fine-tune DeepSeek-Coder model on Oracle FastFormula Q&A data using HuggingFace Transformers.

Usage:
    python train_deepseek_coder.py --output_dir ./deepseek_coder_fastformula_output
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune DeepSeek-Coder on FastFormula data")
    parser.add_argument("--model_name", type=str, default="deepseek-ai/deepseek-coder-6.7b-base", help="Model name or path")
    parser.add_argument("--data_path", type=str, default="../data/fastformula", help="Path to training data (file or directory)")
    parser.add_argument("--output_dir", type=str, default="./deepseek_coder_fastformula_output", help="Output directory")
...
def format_chat_template(prompt, response, tokenizer):
    ...
    # Format as a conversation with special tokens
    text = f"### Instruction:\n{prompt}\n\n### Response:\n{response}"
    return text
...
def main():
    ...
    print("=" * 60)
    print("DeepSeek-Coder FastFormula Fine-tuning")
    print("=" * 60)
    ...
    print(f"Added {len(new_tokens)} custom FastFormula tokens")
    ...
    if len(qa_pairs) == 0:
        raise ValueError("No training data found! Please check your data path.")
